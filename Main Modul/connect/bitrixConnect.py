...
#                if user_active == 'Y':
    ...
    def update_user(self, user_id, new_data, employee, userData):
        ...
        try:
            ...
            if result.get('result'):
                self.send_msg(
                    f"BX24. Обновление данных сотрудника {employee.firstname} {employee.lastname} {employee.surname} из отдела {userData['G2'].value} на должность {userData['J2'].value} ID = {user_id}. Выполнено.")
                return True
        except Exception as e:
            self.send_msg_error(f"BX24. Обновление данных сотрудника {employee.firstname} {employee.lastname} {employee.surname} из отдела {userData['G2'].value} на должность {userData['J2'].value} ID = {user_id} {new_data} Ошибка {e}")
            return False
    ...
    def create_user(self, email, employee, userData):
        ...
        try:
            ...
            if create.get('result'):
                ...
                self.send_msg(
                        f"BX24.Создание: Сотрудник {employee.firstname, employee.lastname, employee.surname} из отдела {userData['G2'].value} на должность {userData['J2'].value} ID = {user_id}. Выполнено")
                return True
            else:
                self.send_msg_error(
                        f"BX24.Создание !Не выполнено: Сотрудник {employee.firstname, employee.lastname, employee.surname} из отдела {userData['G2'].value} на должность {userData['J2'].value}. Ошибка: {create.get('result')}")
                return False
        except Exception as e:
            self.send_msg_error(
