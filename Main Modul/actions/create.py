...
def activate_user_in_AD(conn, user_dn, employee, userData):
    try:
        ...
        bitrix_connector.send_msg_error(
            f"AD. Ошибка при активации учетной записи {employee.firstname} {employee.lastname} {employee.surname} из отдела {userData['G2'].value} на должность {userData['J2'].value}: {e}")
        return False
    finally:
...
#            ('userPassword', [employee.password.encode('utf-16-le')]),
#            ('userPassword', [employee.password.encode('utf-8')]),
    ...
    if flags['AD'] and flags['BX24'] and flags['Normal_account']:
        ...
        if not (existence is None) and len(existence) > 0:
            ...
            if user_info:
                ...
                try:
                    bx24_success = bitrix_connector.bitrix_user_update(bx24, user_info.get('ID'), new_data)
                    bitrix_connector.send_msg(
                        f"BX24. Создание: Обновлении данных сотрудника {employee.firstname} {employee.lastname} {employee.surname} из отдела {userData['G2'].value} на должность {userData['J2'].value} {user_info.get('ID')}. Выполнено.")
                except Exception as e:
                    bx24_success = False
                    bitrix_connector.send_msg_error(
